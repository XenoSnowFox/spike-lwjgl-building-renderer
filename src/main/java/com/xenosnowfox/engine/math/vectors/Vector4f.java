package com.xenosnowfox.engine.math.vectors;

/**
 * A vector composed of 4 floating point values (x, y, z and w).
 */
public class Vector4f {

	/**
	 * X value.
	 */
	public float x;

	/**
	 * Y value.
	 */
	public float y;

	/**
	 * Z value.
	 */
	public float z;

	/**
	 * W value.
	 */
	public float w;

	/**
	 * Instantiates a new instance with an initial zero value.
	 */
	public Vector4f() {
		this(0f, 0f, 0f, 0f);
	}

	/**
	 * Instantiates a new instance.
	 *
	 * @param withX
	 * 		X value
	 * @param withY
	 * 		Y value
	 */
	public Vector4f(final float withX, final float withY, final float withZ, final float withW) {
		this.x = withX;
		this.y = withY;
		this.z = withZ;
		this.w = withW;
	}

	/**
	 * {@inheritDoc}.
	 */
	@Override
	public String toString() {
		return "Vector4f{x=" + this.x + ", y=" + this.y + ", z=" + this.z + ", w=" + this.w + "}";
	}

	/**
	 * Copy constructor.
	 *
	 * @param other
	 * 		instance to copy date from
	 */
	public Vector4f(final Vector4f other) {
		this(other.x, other.y, other.z, other.w);
	}

	/**
	 * Returns the X portion of the vector.
	 *
	 * @return X value
	 */
	public float getX() {
		return this.x;
	}

	/**
	 * Returns the Y portion of the vector.
	 *
	 * @return Y value
	 */
	public float getY() {
		return this.y;
	}

	/**
	 * Returns the Z portion of the vector.
	 *
	 * @return Z value
	 */
	public float getZ() {
		return this.z;
	}

	/**
	 * Returns the W portion of the vector.
	 *
	 * @return W value
	 */
	public float getW() {
		return this.w;
	}

	/**
	 * Defines the X portion of the vector.
	 *
	 * @param withX
	 * 		new X value
	 */
	public void setX(final float withX) {
		this.x = withX;
	}

	/**
	 * Defines the Y portion of the vector.
	 *
	 * @param withY
	 * 		new Y value
	 */
	public void setY(final float withY) {
		this.y = withY;
	}

	/**
	 * Defines the Z portion of the vector.
	 *
	 * @param withZ
	 * 		new Z value
	 */
	public void setZ(final float withZ) {
		this.z = withZ;
	}

	/**
	 * Defines the W portion of the vector.
	 *
	 * @param withW
	 * 		new W value
	 */
	public void setW(final float withW) {
		this.w = withW;
	}

}
